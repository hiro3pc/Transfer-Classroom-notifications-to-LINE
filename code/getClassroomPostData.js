...
function getCourseWorkMaterials() {
    ...
    for (let i = 0; i <= status.length - 1; i++) {
        if (status[i][0] !== "sent") {
            ...
            //データ一部書き換え
            unsentCourseWorkObj.creatorUserName = Classroom.Courses.Teachers.get(COURSE_ID, unsentCourseWorkObj.creatorUserId).profile.name.familyName + "先生"; //先生の姓を取得・敬称をつける
