 ...
 * @return {Array} [タイトルとlinkのテキスト,タイトルとlinkとcolorとファイルの種類を含むJSON,ファイルの種類]
 */
function converteMaterial(materials) {
    ...
    return [materialsTextAndUrlLists, JSON.stringify(materialsObjArray), JSON.stringify(materialUnique)]; //3つめ JSON or 配列をjoin
}
...
function convertTime(time) {
    let date = new Date(parseInt(time.slice(0, 4)), parseInt(time.slice(5, 7)) - 1, parseInt(time.slice(8, 10)), parseInt(time.slice(11, 13)), parseInt(time.slice(14, 16)), parseInt(time.slice(17, 19)), parseInt(time.slice(20, 23)));
    //console.log(time.slice(20,23)) //ミリ秒
    //Europe/London
